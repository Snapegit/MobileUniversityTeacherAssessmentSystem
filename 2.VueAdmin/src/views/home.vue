<template>
<div class="content" :style='{"minHeight":"100vh","width":"100%","padding":"30px","background":"url(http://codegen.caihongy.cn/20240312/db0bfba4b71c42bf91ffc4d5a44e102d.png) no-repeat center top / 100% 100%,#eee","height":"auto"}'>
	<!-- notice -->
	<!-- title -->
	...
	<!-- statis -->
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"space-between","display":"flex"}'>
		<div :style='{"boxShadow":"none","margin":"0 10px","display":"flex"}' v-if="isAuth('pingshichengji','首页总数')">
			<div :style='{"padding":"0 30px  0 40px","alignItems":"center","background":"#fff","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-menu12" :style='{"transform":"rotate(269deg)","borderColor":"#A68DF5","color":"#A68DF5","borderRadius":"9%","borderWidth":"0 0 0 1px","fontSize":"32px","borderStyle":"solid"}'></span>
			</div>
			<div :style='{"padding":"0  40px 0 0","alignItems":"center","flexDirection":"column","background":"#fff","justifyContent":"center","display":"flex"}'>
				<div :style='{"lineHeight":"24px","fontSize":"20px","color":"#A68DF5","fontWeight":"700","height":"24px"}'>{{pingshichengjiCount}}</div>
				<div :style='{"lineHeight":"24px","fontSize":"16px","color":"#A68DF5","height":"24px"}'>平时成绩总数</div>
			</div>
		</div>
		<div :style='{"margin":"0 10px","display":"flex"}' v-if="isAuth('kaoshichengji','首页总数')">
			<div :style='{"padding":"0 30px  0 40px","alignItems":"center","background":"#fff","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-menu12" :style='{"transform":"rotate(269deg)","borderColor":"#A68DF5","color":"#A68DF5","borderRadius":"9%","borderWidth":"0 0 0 1px","fontSize":"32px","borderStyle":"solid"}'></span>
			</div>
			<div :style='{"padding":"0  40px 0 0","alignItems":"center","flexDirection":"column","background":"#fff","justifyContent":"center","display":"flex"}'>
				<div :style='{"lineHeight":"24px","fontSize":"20px","color":"#A68DF5","fontWeight":"700","height":"24px"}'>{{kaoshichengjiCount}}</div>
				<div :style='{"lineHeight":"24px","fontSize":"16px","color":"#A68DF5","height":"24px"}'>考试成绩总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type4" :style='{"alignContent":"flex-start","padding":"10px 20px","flexWrap":"wrap","background":"rebeccapurple","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="pingshichengjiChart1" class="echarts1" v-if="isAuth('pingshichengji','首页统计')"></div>
		<div id="pingshichengjiChart2" class="echarts2" v-if="isAuth('pingshichengji','首页统计')"></div>
		<div id="kaoshichengjiChart1" class="echarts3" v-if="isAuth('kaoshichengji','首页统计')"></div>
		<div id="kaoshichengjiChart2" class="echarts4" v-if="isAuth('kaoshichengji','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	data() {
		return {
            pingshichengjiCount: 0,
            kaoshichengjiCount: 0,
	...
	mounted(){
		this.init();
		this.getpingshichengjiCount();
		this.pingshichengjiChat1();
		this.pingshichengjiChat2();
		this.getkaoshichengjiCount();
		this.kaoshichengjiChat1();
		this.kaoshichengjiChat2();
	},
	methods:{
		// 统计图动画
		...
		getpingshichengjiCount() {
			this.$http({
				url: `pingshichengji/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.pingshichengjiCount = data.data
				}
			})
		},
// 1234 竖
        ...
        var pingshichengjiChart1 = echarts.init(document.getElementById("pingshichengjiChart1"),'roma');
        this.$http({
            url: "pingshichengji/sectionStat/fenshu",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                pingshichengjiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    pingshichengjiChart1.resize();
                };
            }
        });
      })
    ...
    pingshichengjiChat2() {
      this.$nextTick(()=>{
        ...
        var pingshichengjiChart2 = echarts.init(document.getElementById("pingshichengjiChart2"),'roma');
        this.$http({
            url: "pingshichengji/group/chengji",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                pingshichengjiChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    pingshichengjiChart2.resize();
                };
            }
        });
      })
		...
		getkaoshichengjiCount() {
			this.$http({
				url: `kaoshichengji/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.kaoshichengjiCount = data.data
				}
			})
		},
// 1234 折
        ...
        var kaoshichengjiChart1 = echarts.init(document.getElementById("kaoshichengjiChart1"),'roma');
        this.$http({
            url: "kaoshichengji/sectionStat/fenshu",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                kaoshichengjiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    kaoshichengjiChart1.resize();
                };
            }
        });
      })
    ...
    kaoshichengjiChat2() {
      this.$nextTick(()=>{
        ...
        var kaoshichengjiChart2 = echarts.init(document.getElementById("kaoshichengjiChart2"),'roma');
        this.$http({
            url: "kaoshichengji/group/chengji",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                kaoshichengjiChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    kaoshichengjiChart2.resize();
                };
            }
        });
      })
