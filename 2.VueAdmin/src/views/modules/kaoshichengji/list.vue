<template>
	<div class="main-content" :style='{"padding":"30px","margin":"0","background":"#eeeeee"}'>
		<!-- 列表页 -->
			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"justifyContent":"flex-end","display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">考试名称</label>
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="成绩" prop="chengji">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">成绩</label>
						<el-select clearable v-model="searchForm.chengji" placeholder="请选择成绩" >
							<el-option v-for="(item,index) in chengjiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<el-button class="search" type="success" @click="search()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#A68DF5","display":"none","height":"40px"}'></span>
						查询
					</el-button>
				</el-row>

				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('kaoshichengji','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#A68DF5","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('kaoshichengji','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#A68DF5","height":"40px"}'></span>
						删除
					</el-button>


					<download-excel v-if="isAuth('kaoshichengji','导出')" class="export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "考试成绩.xls">
						<!-- 导出excel -->
					...
					<el-button class="btn18" v-if="isAuth('kaoshichengji','考试分数统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#A68DF5","height":"40px"}'></span>
						考试分数统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('kaoshichengji','考试成绩统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#A68DF5","height":"40px"}'></span>
						考试成绩统计
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kaoshichengji','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('kaoshichengji','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('kaoshichengji','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								删除
							</el-button>
						</template>
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var chengjiChart2 = echarts.init(document.getElementById("chengjiChart2"),'roma');
        this.$http({
	...
	kaoshichengjistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'kaoshichengji/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
