<template>
	<div class="main-content" :style='{"padding":"30px","margin":"0","background":"#eeeeee"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('xuesheng','课程评价')" @click="pingshichengjiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								课程评价
							</el-button>
							<el-button class="btn8" v-if="isAuth('xuesheng','考试评价')" @click="kaoshichengjiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								考试评价
							</el-button>
		...
		<pingshichengji-cross-add-or-update v-if="pingshichengjiCrossAddOrUpdateFlag" :parent="this" ref="pingshichengjiCrossaddOrUpdate"></pingshichengji-cross-add-or-update>
...
import pingshichengjiCrossAddOrUpdate from "../pingshichengji/add-or-update";
	...
	export default {
		data() {
			return {
				...
				pingshichengjiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			pingshichengjiCrossAddOrUpdate,
		...
		methods: {
			...
			pingshichengjiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.pingshichengjiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.pingshichengjiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.pingshichengjiCrossaddOrUpdate.init(row.id,type);
				});
			},
			kaoshichengjiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
